...
async function chainPromises(array, action) {
  const arr = [];
  array.forEach((item) => item.then((data) => arr.push(data)).catch((err) => new Error(err)));
  const arr2 = await arr;
  const result = await new Promise((resolve) => resolve(arr2.reduce(action)));
  return result;
}
